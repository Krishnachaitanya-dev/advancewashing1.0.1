...
import { Shirt, Zap, Bed, Shield, Package, ShoppingBag, CheckCircle2 } from 'lucide-react';
const iconMap: {
...
const serviceColors = ['border-green-400', 'border-pink-400', 'border-violet-400', 'border-orange-400', 'border-yellow-400', 'border-cyan-400'];
const serviceBgColors = ['bg-green-400/30', 'bg-pink-400/30', 'bg-violet-400/30', 'bg-orange-400/30', 'bg-yellow-400/30', 'bg-cyan-400/30'];
const serviceCheckColors = ['text-green-400', 'text-pink-400', 'text-violet-400', 'text-orange-400', 'text-yellow-400', 'text-cyan-400'];
  ...
  return <AppLayout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          return <div key={service.id} onClick={() => handleServiceSelect(service)} className={getServiceStyle(service.id)}>
                {/* Selection Indicator */}
                {isSelected && <div className="absolute -top-2 -right-2 z-10">
                    <div className="bg-white rounded-full p-1 shadow-lg">
                      <CheckCircle2 className={`w-6 h-6 ${getCheckmarkColor(service.id)} fill-current`} />
                    </div>
                  </div>}
        ...
        <div className="pb-6">
          <Button onClick={handleSchedulePickup} disabled={selectedServices.length === 0} className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white py-4 rounded-xl font-medium text-lg shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
            Schedule Pickup
          </Button>
        </div>
      </div>
