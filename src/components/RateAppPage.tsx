      ...
      <div className="glass-card p-6">
        ...
        <div className="mb-8">
          ...
          <div className="flex justify-center space-x-2">
            {[1, 2, 3, 4, 5].map(star => <button key={star} onClick={() => handleStarClick(star)} className="transition-colors">
                <Star size={40} className={star <= rating ? 'text-yellow-400 fill-current' : 'text-white/30'} />
              </button>)}
          </div>
          {rating > 0 && <p className="text-center text-white/70 mt-2">
        ...
        <div className="mb-6">
          ...
          <textarea value={feedback} onChange={e => setFeedback(e.target.value)} placeholder="Share your thoughts about the app, suggestions for improvement, or what you loved most..." className="w-full p-4 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/50 resize-none h-32 focus:outline-none focus:border-white/40" maxLength={500} />
          <p className="text-white/50 text-sm mt-1">{feedback.length}/500 characters</p>
        </div>

        <div className="space-y-3">
          <Button onClick={handleSubmit} disabled={rating === 0} className="w-full bg-blue-900 hover:bg-blue-800 text-white disabled:opacity-50 disabled:cursor-not-allowed">
            Submit Rating
          </Button>
