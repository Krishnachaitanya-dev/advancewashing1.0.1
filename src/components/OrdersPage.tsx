  ...
  return <AppLayout>
      <div className="space-y-6">
        ...
        {orders.length === 0 ? <div className="glass-card p-8 text-center">
            ...
            {orders.map(order => <div key={order.id} className="glass-card p-4 cursor-pointer hover:bg-white/5 transition-colors" onClick={() => handleOrderClick(order)}>
                ...
                <div className="space-y-2 mb-4">
                  ...
                  {order.final_weight && <div className="text-white/80 text-sm">
                      <strong>Weight:</strong> {order.final_weight} kg
                    </div>}
                  
                  {order.final_price && order.final_weight && (order.status === 'delivered' || order.status === 'ready_for_delivery') ? <div className="text-white/80 text-sm">
                      <strong>Total:</strong> ₹{order.final_price}
                    </div> : <div className="text-white/80 text-sm">
                      <strong>Price:</strong> Will be calculated after weighing
                    </div>}
                  
                  {order.bookings?.pickup_time && <div className="text-white/80 text-sm">
                ...
                return acc;
              }, {} as Record<string, any>) || {}).map(([key, item]) => <div key={key} className="flex justify-between text-white/80 text-sm">
                        <span>
                          {item.name}
                        </span>
                        <span>
                ...
                {order.status === 'pending' && <div className="mt-4 pt-3 border-t border-white/20">
                    <Button variant="outline" size="sm" className="border-red-500/50 text-red-300 hover:bg-red-500/20" onClick={e => {
              e.stopPropagation();
              // Handle cancel order
            }}>
                      Cancel Order
